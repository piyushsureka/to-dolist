...
addtaskbtn.addEventListener("click", function () {
    ...
    if (addtaskinputval.length === 0) {
        showAlert("Please enter any item first.", "alert-danger");
        return;
    }
...
function showtask() {
    ...
    taskObj.forEach((item, index) => {
        ...
        html += `<tr>
                    ...
                    <td><button type="button" class="text-success" id=${index}>
                    <i class="bi bi-check-square"></i></i>Complete</button></td>
...
deleteallbtn.addEventListener("click", function () {
    ...
    if (confirm("Are you sure want to delete?") === true) {
        ...
        addtaskbtn.style.display = "block";
        localStorage.setItem("localtask", JSON.stringify(taskObj));
        showAlert("All the Item has been deleted.", "alert-success");
        showtask();
    }
})
